...
function displayCategories (categories){
    ...
    for (const category of categories){
        ...
        catDiv.innerHTML = `
        <button id="btn-${category.category_id}" onclick="loadCatVideos(${category.category_id})" class="btn btn-sm hover:bg-red-800 hover:text-white">${category.category}</button>`
        ;
